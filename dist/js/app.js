(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".img__slider")) {
                const imgSlider = new core(".img__slider", {
                    modules: [ Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4,
                ...
                const countrys = document.querySelector(".main__countrys");
                let items = countrys.children;
                imgSlider.on("activeIndexChange", (function() {
                    for (let i = 0; i < items.length; i++) {
                        items[i].classList.remove("active");
                        if (items[i].dataset.index == imgSlider.realIndex) items[i].classList.add("active");
                    }
                }));
            }
        }
        ...
        function map() {
            const rightWhereLabel = document.querySelectorAll(".right-where__label");
            const rightWhereLocation = document.querySelectorAll(".right-where__location");
            const rightWhere = document.querySelector(".right-where__bottom");
            if (document.querySelector("#kyiv")) {
                let xKyiv = document.querySelector("#kyiv").offsetLeft;
                let yKyiv = document.querySelector("#kyiv").offsetTop;
                const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
                svg.setAttributeNS(null, "viewBox", ``);
                let pathTransparent = document.createElementNS("http://www.w3.org/2000/svg", "path");
                let pathColor = document.createElementNS("http://www.w3.org/2000/svg", "path");
                pathTransparent.classList.add("line-transparent");
                pathColor.classList.add("line-color", "line-color-1");
                let x = document.querySelector("#ecuador").offsetLeft;
                let y = document.querySelector("#ecuador").offsetTop;
                let plane = document.querySelector("#ecuador").children[2];
                pathTransparent.setAttribute("d", `M${x} ${y} L${xKyiv} ${yKyiv}`);
                pathTransparent.setAttribute("fill", "transparent");
                pathTransparent.setAttribute("stroke", "transparent");
                pathTransparent.setAttribute("stroke-width", "2");
                pathColor.setAttribute("d", `M${x} ${y} L${xKyiv} ${yKyiv}`);
                pathColor.setAttribute("fill", "#ffffff");
                pathColor.setAttribute("stroke", "#ffffff");
                pathColor.setAttribute("stroke-width", "2");
                let xPlane = `${xKyiv}` - `${x}`;
                let yPlane = `${yKyiv}` - `${y}`;
                plane.style.cssText = `\n\t\toffset-path : path('M0 0 L${xPlane} ${yPlane}');\n\t\t`;
                svg.appendChild(pathTransparent);
                svg.appendChild(pathColor);
                rightWhere.appendChild(svg);
                window.addEventListener("click", (e => {
                    if (e.target.parentNode.classList.contains("right-where__label") || e.target.parentNode.classList.contains("right-where__location")) for (let i = 0; i < rightWhereLabel.length; i++) {
                        rightWhereLabel[i].classList.remove("active");
                        rightWhereLocation[i].classList.remove("active");
                        e.target.parentNode.classList.add("active");
                        if (rightWhereLabel[i].classList.contains("active") || rightWhereLocation[i].classList.contains("active")) {
                            rightWhereLocation[i].classList.add("active");
                            let plane = rightWhereLocation[i].children[2];
                            rightWhereLabel[i].classList.add("active");
                            let x = rightWhereLocation[i].offsetLeft;
                            let y = rightWhereLocation[i].offsetTop;
                            pathTransparent.setAttribute("d", `M${x} ${y} L${xKyiv} ${yKyiv}`);
                            pathTransparent.setAttribute("fill", "transparent");
                            pathTransparent.setAttribute("stroke", "transparent");
                            pathTransparent.setAttribute("stroke-width", "2");
